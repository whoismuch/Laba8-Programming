...
public class UserManager {
    ...
    public String checkRoute (Locale locale, int now, String name, String coordX, String coordY, String fromName, String fromX, String fromY, String toName, String toX, String toY, String distance) {
        try {
            ...
            if (Math.abs(r_fromX) > 2000L) return "Координата X места отправления должна быть <= 2000";
            now = 6;
            Long r_fromY = Long.parseLong(fromY);
            if (Math.abs(r_fromY) > 2000L) return "Координата Y места отправления должна быть <= 2000";
            now = 7;
            String r_toName = toName;
            now = 8;
            long r_toX = Long.parseLong(toX);
            if (Math.abs(r_toX) > 2000L) return "Координата X места прибытия должна быть <= 2000";
            now = 9;
            Long r_toY = Long.parseLong(toY);
            if (Math.abs(r_toY) > 2000L) return "Координата Y места прибытия должна быть <= 2000";
            now = 10;
